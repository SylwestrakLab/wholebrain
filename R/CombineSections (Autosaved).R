...
filename <- file.path(getwd(),genotype,'CompositeData',paste(mouse, '_composite_plot.pdf',sep = "", collapse=NULL))
quartz.save(filename,type="pdf")
save(datasets, file=file.path(getwd(),genotype, 'CompositeData',paste(mouse, '_compositeData.Rdata',sep = "", collapse=NULL)))
