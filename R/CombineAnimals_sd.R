...
if (length(firstMouse)==1) {
  load(firstMouse[1])
  datasets <- datasets[,!names(datasets) %in% c("scaleFactor", "somaX", "somaY")]
...
row.names.remove <- c("MDm","CA1sr","LH", "MH","MPT","TH", "PVT","fiber tracts","TH","PF","PRC","V3","IMD","grey")
cohortData<-cohortData[!(cohortData$acronym %in% row.names.remove), ]
regions <- unique(cohortData$acronym)
d = matrix(0, nrow = length(regions), ncol = length(genotypes))
s = matrix(0, nrow = length(regions), ncol = length(genotypes))

for(c in 1:length(genotypes)){
  thisGenotype = subset(cohortData, genotype == genotypes[c])
  animalList = unique(thisGenotype$animal)
  nAnimals <- length(unique(thisGenotype$animal))
  regionAnimalMatrix = matrix(0, nrow = length(regions), ncol = nAnimals)
  regionAnimalMatrix_s = matrix(0, nrow = length(regions), ncol = nAnimals)
  allCells = matrix(0, nrow = nAnimals, ncol = 1)
  for (a in 1:nAnimals){
    
    allCells[a]<-nrow(thisGenotype[thisGenotype$animal == animalList[a], ])
    thisAnimal<- subset(thisGenotype, animal == animalList[a])
    for(i in 1:length(regions)){
      regionAnimalMatrix[i, a]<-nrow(subset(thisAnimal, acronym == regions[i]))/allCells[a]*100
      #d[i, c+1]<- area$color[1]
    }
    if(ncol(regionAnimalMatrix)>1){
      regionAnimalMatrix_m<-rowMeans(regionAnimalMatrix, na.rm=TRUE)
      regionAnimalMatrix_s <- apply(regionAnimalMatrix, 1, sd, na.rm = TRUE) 
    } else {
        regionAnimalMatrix_m<-regionAnimalMatrix
    }
  }
  d[, c]<-regionAnimalMatrix_m
  s[, c]<-regionAnimalMatrix_s/(nAnimals)
}
colnames(d) <- genotypes
rownames(d) <- regions

rownames(regionAnimalMatrix) <- regions

cohort <- as.table(d)
row.names.remove <- c("MDm","LH", "MH","MPT","TH", "PVT","fiber tracts","TH","PF","PRC","V3","IMD","grey")
...
g<-c(1,2,3,4,5,6,7)
counts.compare.plot<-function (cohort, sd, device = TRUE, region.lab = "Input region:", 
                               xlab= 'Pct. of Cells', countThreshold = 3, sorted = FALSE, cellColors=Colors, genotypesToRun=g)
{
  ...
  for (cc in genotypesToRun) {
    ...
    err = sd[which(idx), cc]
    segments(counts[which(idx), cc]-err,nrow(counts)+1-which(idx), counts[which(idx), cc]+err,nrow(counts)+1-which(idx),
           pch = 21, col = cellColors[cc], cex = 1.2)
  }
  box()
  par(xpd = TRUE)
...
counts.compare.plot(cohort, s, device = TRUE, region.lab = "Input region:", xlab= 'Pct. of Cells', countThreshold = 1, sorted = TRUE) 
